...
void UART0_Init(void)
{
	...
	UART0->IBRD = 520;									// IBRD = int(80,000,000 / (16 * 9,600)) = int(43.402777)
	UART0->FBRD = 54;									// FBRD = int(0.402777 * 64 + 0.5) = round(26.2777)
	...
	NVIC_SetPriority(UART0_IRQn, (1<<5));	// UART0=priority 2
...
static void copyHardwareToSoftware(void)
{
	...
	while (((UART0->FR & UART_FR_RXFE) == 0) && (Rx0Fifo_Size() < (FIFOSIZE - 1)))
	{
		letter = UART0->DR;
		Rx0Fifo_Put(letter);
	}
...
static void copySoftwareToHardware(void)
{
	...
	while (((UART0->FR & UART_FR_TXFF) == 0) && (Tx0Fifo_Size() > 0))
	{
		Tx0Fifo_Get(&letter);
		UART0->DR = letter;
		if (letter ==  '\n')
